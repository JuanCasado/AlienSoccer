...
#define PIN_TRIG_US_1 2
#define PIN_ECHO_US_1 3
...
int medida = 0, medida_1 = 0, medida_2 = 0, medida_3 = 0, medida_4 = 0;
byte continuo = false;
...
void setup()
{
  pinMode(PIN_TRIG_US_1, OUTPUT);
  pinMode(PIN_ECHO_US_1, INPUT);
...
{
  //Medida 1
  if (((millis() - crono) > ESPERA_ENTRE_MEDIDAS) && (sensor == 1))
  {
    medida = mide_us(PIN_TRIG_US_2, PIN_ECHO_US_2);
    if (medida > 0)
    {
      medida_1 = constrain(medida, 0, 255);    
    }
    sensor = 2;
    crono = millis();
  }
  ...
  {
    medida = mide_us(PIN_TRIG_US_1, PIN_ECHO_US_1);
    if (medida > 0)
    {
