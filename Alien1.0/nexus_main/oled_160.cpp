...
void oled_texto(char *mensaje, unsigned char columna, unsigned char fila, unsigned char fuente, int color)
{
  ...
  Serial2.print('s');	
  Serial2.write((byte)columna);
  Serial2.write((byte)fila);
  //fuente (0/1/2)
  Serial2.write((byte)fuente);
  oled_color(color);

  for (int i=0 ; i<strlen(mensaje) ; i++)
    Serial2.write((byte)mensaje[i]); // character to write
...
{
  Serial2.print('b');
  Serial2.write((byte)estado);
  Serial2.write((byte)x);
  Serial2.write((byte)y);
  //color del boton
  Serial2.write((byte)0x00);
  Serial2.write((byte)0x1F);
  //fuente grande
...
{
  Serial2.print('r');
  Serial2.write((byte)x1);
  Serial2.write((byte)y1);
  Serial2.write((byte)x2);
  Serial2.write((byte)y2);
...
void oled_color(int color)
{
  ...
  Serial2.write((byte)color_alto);
  Serial2.write((byte)color_bajo);
}

void oled_fuente(char tamano)
{
  Serial2.print('F');
  Serial2.write((byte)tamano);
  //delay(1);
  oled_espera_respuesta();
}
